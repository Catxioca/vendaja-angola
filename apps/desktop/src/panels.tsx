...
export function CashPanel({ api, token, onError, currentUser, authorizeOffline, onSessionExpired }: Omit<Props, "admin">) {
  ...
  const open = async () => { if (!/^\d{6}$/.test(pin)) return onError("Introduza o PIN de 6 dígitos do administrador."); if (operatorName.trim().length < 2 || operatorNif.trim().length < 5 || operatorPhone.trim().length < 7) return onError("Preencha nome, NIF e telefone do atendedor."); try { let opened: { id: string; operatorName?: string; operatorNif?: string; operatorPhone?: string; terminalId?: string }; try { if (token === "offline-local-session") throw new Error("BACKEND_UNAVAILABLE"); opened = await request<typeof opened>(`${api}/api/v1/fiscal/cash/open`, token, { method: "POST", body: JSON.stringify({ pin, cashierName: operatorName, cashierNif: operatorNif, cashierPhone: operatorPhone, registerNumber: terminalId, initialAmount: Number(opening.replace(",", ".")) || 0, openingCents: money(opening) }) }); } catch (error) { if (error instanceof Error && error.name === "HTTP_401") { onSessionExpired?.(); throw new Error("Sessão expirada. Por favor, faça login novamente."); } if (!(error instanceof Error && error.message === "BACKEND_UNAVAILABLE")) throw error; if (!authorizeOffline) throw new Error("Não foi possível validar o PIN offline."); await authorizeOffline(pin); opened = { id: `local-cash-${Date.now()}`, operatorName, operatorNif, operatorPhone, terminalId }; } const local = { id: opened.id, openingCents: money(opening), sangriaCents: 0, suprimentoCents: 0, openedAt: new Date().toISOString(), operatorName, operatorNif, operatorPhone, terminalId, summary: { salesCount: 0, totalCents: 0, cashCents: 0, cardCents: 0, transferCents: 0, expectedCashCents: money(opening) } }; localStorage.setItem(localCashKey, JSON.stringify(local)); localStorage.setItem("activeCashSessionId", opened.id); localStorage.setItem("terminalId", terminalId); localStorage.setItem("cashOperatorNif", operatorNif); localStorage.setItem("cashOperatorPhone", operatorPhone); setPin(""); setOpening(""); await load(); } catch (e) { const message = e instanceof Error ? e.message : "Falha ao abrir caixa."; onError(message === "BACKEND_UNAVAILABLE" ? `Não foi possível conectar ao servidor backend (${api}).` : message === "invalid_pin" ? "PIN inválido." : message); } };
