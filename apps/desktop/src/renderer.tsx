...
import { AccountingPanel, CashPanel, CashflowPanel, DashboardPanel, DevicesPanel, FiscalSettingsPanel, HRPanel, OperationsPanel, PlatformPanel, ProductsPanel, ReportsPanel, StockDocumentsPanel } from "./panels";
...
type View = "pdv" | "dashboard" | "products" | "stock-documents" | "cash" | "fiscal" | "devices" | "hr" | "accounting" | "operations" | "reports" | "cashflow" | "platform";
type Payment = { method: PaymentMethod | "MIXED"; cashCents: number; cardCents: number; transferCents: number; customerId: string; customerName: string; customerNif: string; dueDate: string };
type CustomerOption = { id: string; name: string; nif?: string | null; creditLimitCents: number; outstandingDebtCents: number; active: boolean };
...
function App() {
  ...
  const [payment, setPayment] = useState<Payment>({ method: "CASH", cashCents: 0, cardCents: 0, transferCents: 0, customerId: "", customerName: "", customerNif: "", dueDate: "" });
  ...
  useEffect(() => {
    if (!token || token === "offline-local-session") return;
    void fetch(`${api()}/api/v1/customers`, { headers: { authorization: `Bearer ${token}` } })
      .then((response) => response.ok ? response.json() as Promise<CustomerOption[]> : Promise.reject(new Error("customers_unavailable")))
      .then((loaded) => setCustomers(loaded.filter((customer) => customer.active)))
      .catch(() => undefined);
  }, [token]);
  ...
  async function confirmPayment() {
    ...
    if (payment.method === "CREDIT" && (!payment.customerId || !payment.dueDate)) { setError("Selecione um cliente e uma data de vencimento para vender a crédito."); return; }
    if (payment.method !== "CREDIT" && documentType !== "PROFORMA" && paid < total) { setError("O valor recebido é inferior ao total."); return; }
    ...
    const sale: Sale = {
      ...
      discountCents, paymentMethod: documentType === "PROFORMA" ? undefined : payment.method, cashCents: documentType === "PROFORMA" || payment.method === "CREDIT" ? 0 : payment.cashCents, cardCents: documentType === "PROFORMA" || payment.method === "CREDIT" ? 0 : payment.cardCents, transferCents: documentType === "PROFORMA" || payment.method === "CREDIT" ? 0 : payment.transferCents, cashSessionId: documentType === "PROFORMA" || payment.method === "CREDIT" ? null : cashSessionId, customerId: payment.customerId || null, dueDate: payment.method === "CREDIT" ? new Date(payment.dueDate).toISOString() : null, customerName: payment.customerName || undefined, customerNif: payment.customerNif || undefined, fiscalType: documentType,       operatorName: localStorage.getItem("cashOperatorName") || session?.displayName, terminalId: localStorage.getItem("terminalId") || terminalId, operatorNif: localStorage.getItem("cashOperatorNif") ?? undefined, operatorPhone: localStorage.getItem("cashOperatorPhone") ?? undefined,
    };
    ...
    try {
      ...
      setCompletedSale({ sale, hash, qr: fiscalQrPayload(sale, hash) }); setCart({}); setDiscount(0); setModal(null); setPayment({ method: "CASH", cashCents: 0, cardCents: 0, transferCents: 0, customerId: "", customerName: "", customerNif: "", dueDate: "" }); window.dispatchEvent(new CustomEvent("cash-updated"));
  ...
  return <main>
    ...
    <nav aria-label="Navegação principal">{[
      ...
      ...(normalizeRole(session.role) === "ROLE_ADMIN" || (licenseModules.includes("STOCK") && roleModules[normalizeRole(session.role)]?.includes("STOCK")) ? [["stock-documents", "Inventário / Documentos"]] : []),
      ...
      ...(normalizeRole(session.role) === "ROLE_ADMIN" || (licenseModules.includes("STOCK") && roleModules[normalizeRole(session.role)]?.includes("STOCK")) ? [["operations", "Fornecedores / Compras"]] : []),
      ...(roleModules[normalizeRole(session.role)]?.includes("DASHBOARD") ? [["reports", "Relatórios"]] : []),
      ...(roleModules[normalizeRole(session.role)]?.includes("ACCOUNTING") ? [["cashflow", "Fluxo de caixa"]] : []),
      ...(normalizeRole(session.role) === "ROLE_ADMIN" || licenseModules.includes("SETTINGS") ? [["devices", "Dispositivos"]] : []),
      ...(normalizeRole(session.role) === "ROLE_ADMIN" ? [["platform", "Empresa / POS"]] : []),
      ["fiscal", "Definições"],
      ...
      else if (key === "reports") openRestrictedModule("reports", "DASHBOARD");
      else if (key === "cashflow") openRestrictedModule("cashflow", "ACCOUNTING");
      else if (key === "platform") openRestrictedModule("platform", "SETTINGS");
