...
function App() {
  ...
  const [modal, setModal] = useState<"payment" | "cancel" | "profile" | null>(null);
  ...
  return <main>
    ...
    <nav aria-label="Navegação principal">{[["pdv", "PDV"], ["dashboard", "Dashboard"], ...(session.role.toUpperCase() === "ADMIN" ? [["products", "Produtos / Stock"], ["cash", "Caixa"], ["fiscal", "Definições fiscais"], ["devices", "Dispositivos"], ["hr", "RH"], ["accounting", "Contabilidade"]] : [])].map(([key, label]) => <button className={view === key ? "active" : ""} key={key} onClick={() => setView(key as View)}>{label}</button>)}</nav>
