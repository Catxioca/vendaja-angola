...
function App() {
  ...
  const openSettings = () => {
    if (session.role.toUpperCase() === "ADMIN" || settingsUnlocked) { setView("fiscal"); return; }
  ...
  const authorizeSettings = async () => {
    ...
    try {
      if (token === "offline-local-session") {
        ...
        const response = await fetch(`${api()}/api/v1/auth/authorize`, { method: "POST", headers: { "content-type": "application/json", authorization: `Bearer ${token}` }, body: JSON.stringify({ credential, mode: /^\d{6}$/.test(credential) ? "pin" : "password", action: "SETTINGS_ACCESS" }) });
        const data = await response.json().catch(() => ({}));
        if (!response.ok) throw new Error(data.message ?? "Credenciais do Administrador inválidas.");
      }
      setSettingsUnlocked(true); setModal(null); setView("fiscal");
  ...
  const taxSummary = [0, 0.05, 0.14].map((rate) => {
    const matching = taxLines.filter((line) => line.taxRate === rate);
  ...
  return <main>
    ...
    <nav aria-label="Navegação principal">{[["pdv", "PDV"], ["dashboard", "Dashboard"], ...(session.role.toUpperCase() === "ADMIN" ? [["products", "Produtos / Stock"], ["cash", "Caixa"], ["hr", "RH"], ["accounting", "Contabilidade"]] : []), ...((session.role.toUpperCase() === "ADMIN" || settingsUnlocked) ? [["devices", "Dispositivos"]] : []), ["fiscal", "Definições"]].map(([key, label]) => <button className={view === key ? "active" : ""} key={key} onClick={() => key === "fiscal" || key === "devices" ? (settingsUnlocked || session.role.toUpperCase() === "ADMIN" ? setView(key as View) : openSettings()) : setView(key as View)}>{label}</button>)}</nav>
