...
function CompanyContextSelector() {
  ...
  const [value, setValue] = useState(`${localStorage.getItem("vendaja-company-id") ?? ""}|${localStorage.getItem("vendaja-branch-id") ?? ""}`);
  useEffect(() => { void fetch(`${api()}/api/v1/platform/context`, { headers: { authorization: `Bearer ${localStorage.getItem("accessToken") ?? ""}` } }).then(response => response.ok ? response.json() as Promise<typeof contexts> : Promise.reject(new Error("context_unavailable"))).then(setContexts).catch(() => setContexts([])); }, []);
  const select = (next: string) => { const [companyId, branchId] = next.split("|"); setValue(next); if (companyId) localStorage.setItem("vendaja-company-id", companyId); if (branchId) localStorage.setItem("vendaja-branch-id", branchId); };
  return <section><h2>Empresa e filial</h2><label>Contexto autorizado<select value={value} onChange={event => select(event.target.value)}><option value="">Selecionar contexto</option>{contexts.map(context => <option key={`${context.companyId}|${context.branchId ?? ""}`} value={`${context.companyId}|${context.branchId ?? ""}`}>{context.company.legalName} · {context.branch?.name ?? "Todas as filiais"}</option>)}</select></label><p><small>O contexto selecionado apenas identifica uma associação autorizada; os headers nunca concedem acesso.</small></p></section>;
}
type DashboardData = {
...
function AccountingOperations() {
  const [statement, setStatement] = useState<{ balanceSheet: Array<{ code: string; name: string; balanceCents: number }>; incomeStatement: Array<{ code: string; name: string; balanceCents: number }>; totals: { revenueCents: number; expensesCents: number } } | null>(null);
  const [periods, setPeriods] = useState<Array<{ id: string; name: string; status: string; startsAt: string; endsAt: string }>>([]);
  const [error, setError] = useState("");
  const load = async () => {
    const headers = { authorization: `Bearer ${localStorage.getItem("accessToken") ?? ""}` };
    const [statementResponse, periodsResponse] = await Promise.all([fetch(`${api()}/api/v1/accounting/financial-statements`, { headers }), fetch(`${api()}/api/v1/accounting/periods`, { headers })]);
    if (!statementResponse.ok || !periodsResponse.ok) { setError("Não foi possível carregar a contabilidade."); return; }
    setStatement(await statementResponse.json() as typeof statement); setPeriods(await periodsResponse.json() as typeof periods);
  };
  useEffect(() => { void load(); }, []);
  return <section><h2>Contabilidade e fiscalidade</h2>{error && <p className="error-message">{error}</p>}<div className="cards"><article><small>Receitas</small><strong>{formatKwanza(statement?.totals.revenueCents ?? 0)}</strong></article><article><small>Despesas</small><strong>{formatKwanza(statement?.totals.expensesCents ?? 0)}</strong></article></div><h3>Períodos contabilísticos</h3><table><thead><tr><th>Período</th><th>Início</th><th>Fim</th><th>Estado</th></tr></thead><tbody>{periods.map(period => <tr key={period.id}><td>{period.name}</td><td>{new Date(period.startsAt).toLocaleDateString("pt-AO")}</td><td>{new Date(period.endsAt).toLocaleDateString("pt-AO")}</td><td>{period.status}</td></tr>)}</tbody></table><h3>Balancete — resultados</h3><table><thead><tr><th>Conta</th><th>Descrição</th><th>Saldo</th></tr></thead><tbody>{statement?.incomeStatement.map(row => <tr key={row.code}><td>{row.code}</td><td>{row.name}</td><td>{formatKwanza(row.balanceCents)}</td></tr>)}</tbody></table></section>;
}
function Cashflow() {
...
function StockOperations({ online }: { online: boolean }) {
  type Tab = "overview" | "inventory" | "transfer" | "receipt" | "returns" | "procurement" | "quotation" | "order";
  const [tab, setTab] = useState<Tab>("overview");
  const [products, setProducts] = useState<Array<{ id: string; name: string; sku: string; stock: number; priceCents: number }>>([]);
  const [locations, setLocations] = useState<Array<{ id: string; name: string }>>([]);
  const [balances, setBalances] = useState<Array<{ product: { name: string }; location: { name: string }; quantity: number; averageCostCents: number; totalCostCents: number }>>([]);
  const [suggestions, setSuggestions] = useState<Array<{ name: string; currentStock: number; minStock: number; suggestedQuantity: number }>>([]);
  const [purchases, setPurchases] = useState<Array<{ id: string; number: string }>>([]);
  const [requests, setRequests] = useState<Array<{ number: string; lines: Array<{ productId: string; quantity: number }>; quotations: Array<{ number: string; supplierId: string; status: string; lines: Array<{ productId: string; unitCostCents: number }> }> }>>([]);
  const [form, setForm] = useState({ productId: "", quantity: "1", reference: "", locationId: "", sourceLocationId: "", destinationLocationId: "", purchaseId: "", documentType: "CUSTOMER_RETURN", series: "A", number: "", customerId: "", supplierId: "", lotNumber: "", serialNumber: "", expiresAt: "", requestNumber: "", quotationNumber: "", orderNumber: "", unitCostCents: "0" });
  const [message, setMessage] = useState("");
  const auth = { authorization: `******"accessToken") ?? ""}`, "content-type": "application/json" };
  const update = (key: keyof typeof form, value: string) => setForm(current => ({ ...current, [key]: value }));
  const load = async () => {
    if (!online) return;
    try {
      const [productResponse, warehouseResponse, balanceResponse, suggestionResponse, purchaseResponse, requestResponse] = await Promise.all([
        fetch(`${api()}/api/v1/products`, { headers: auth }), fetch(`${api()}/api/v1/stock/warehouses`, { headers: auth }),
        fetch(`${api()}/api/v1/stock/balances`, { headers: auth }), fetch(`${api()}/api/v1/stock/reorder-suggestions`, { headers: auth }), fetch(`${api()}/api/v1/purchases`, { headers: auth }), fetch(`${api()}/api/v1/procurement/requests`, { headers: auth }),
      ]);
      if (![productResponse, warehouseResponse, balanceResponse, suggestionResponse, purchaseResponse, requestResponse].every(response => response.ok)) throw new Error("Não foi possível carregar stock e compras.");
      const warehouses = await warehouseResponse.json() as Array<{ locations: Array<{ id: string; name: string }> }>;
      setProducts(await productResponse.json()); setLocations(warehouses.flatMap(warehouse => warehouse.locations)); setBalances(await balanceResponse.json()); setSuggestions(await suggestionResponse.json()); setPurchases(await purchaseResponse.json()); setRequests(await requestResponse.json());
    } catch (error) { setMessage(error instanceof Error ? error.message : "Falha ao carregar stock."); }
  };
  useEffect(() => { void load(); }, [online]);
  const submit = async () => {
    try {
      const quantity = Number(form.quantity); let endpoint = ""; let body: unknown;
      ...
      else if (tab === "quotation") { endpoint = "/api/v1/procurement/quotations"; body = { number: form.quotationNumber, supplierId: form.supplierId, lines: [{ productId: form.productId, quantity, unitCostCents: Number(form.unitCostCents) }] }; }
      else { endpoint = "/api/v1/procurement/orders"; body = { number: form.orderNumber, supplierId: form.supplierId, lines: [{ productId: form.productId, quantity, unitCostCents: Number(form.unitCostCents) }] }; }
      const response = await fetch(`${api()}${endpoint}`, { method: "POST", headers: auth, body: JSON.stringify(body) });
      if (!response.ok) { const data = await response.json().catch(() => ({})); throw new Error(data.error ?? "Operação rejeitada."); }
      setMessage("Operação concluída e auditada."); await load();
    } catch (error) { setMessage(error instanceof Error ? error.message : "Falha na operação."); }
  };
  const select = (label: string, key: keyof typeof form, items: Array<{ id: string; label: string }>) => <label>{label}<select value={form[key]} onChange={event => update(key, event.target.value)}><option value="">Selecione</option>{items.map(item => <option key={item.id} value={item.id}>{item.label}</option>)}</select></label>;
  return <section><h2>Stock, compras e documentos</h2><div className="customer-fields">{(["overview", "inventory", "transfer", "receipt", "returns", "procurement", "quotation", "order"] as Tab[]).map(item => <button key={item} onClick={() => setTab(item)}>{item === "overview" ? "Resumo" : item === "inventory" ? "Inventário" : item === "transfer" ? "Transferência" : item === "receipt" ? "Receções" : item === "returns" ? "Devoluções" : item === "procurement" ? "Pedido de cotação" : item === "quotation" ? "Cotação" : "Ordem de compra"}</button>)}</div>{!online && <p className="offline-notice">Operações ficam disponíveis quando a ligação for restabelecida; vendas locais continuam na fila offline.</p>}{message && <p>{message}</p>}{tab === "overview" ? <><h3>Alertas de reposição</h3><table><thead><tr><th>Produto</th><th>Atual</th><th>Mínimo</th><th>Sugerido</th></tr></thead><tbody>{suggestions.map(item => <tr key={item.name}><td>{item.name}</td><td>{item.currentStock}</td><td>{item.minStock}</td><td>{item.suggestedQuantity}</td></tr>)}</tbody></table><h3>Valorização por custo médio</h3><table><thead><tr><th>Produto</th><th>Localização</th><th>Qtd.</th><th>Custo médio</th><th>Valor</th></tr></thead><tbody>{balances.map((item, index) => <tr key={`${item.product.name}-${index}`}><td>{item.product.name}</td><td>{item.location.name}</td><td>{item.quantity}</td><td>{formatKwanza(item.averageCostCents)}</td><td>{formatKwanza(item.totalCostCents)}</td></tr>)}</tbody></table></> : <div className="customer-fields">{select("Produto", "productId", products.map(product => ({ id: product.id, label: `${product.name} · ${product.sku}` })))}{tab === "transfer" ? <>{select("Origem", "sourceLocationId", locations.map(location => ({ id: location.id, label: location.name })))}{select("Destino", "destinationLocationId", locations.map(location => ({ id: location.id, label: location.name })))}</> : select("Localização", "locationId", locations.map(location => ({ id: location.id, label: location.name })))}<label>Quantidade<input type="number" min="1" value={form.quantity} onChange={event => update("quantity", event.target.value)} /></label>{(tab === "inventory" || tab === "transfer" || tab === "receipt") && <label>Referência<input value={form.reference} onChange={event => update("reference", event.target.value)} /></label>}{tab === "receipt" && select("Compra", "purchaseId", purchases.map(purchase => ({ id: purchase.id, label: purchase.number })))}{tab === "returns" && <><label>Tipo<select value={form.documentType} onChange={event => update("documentType", event.target.value)}><option value="CUSTOMER_RETURN">Devolução de cliente</option><option value="SUPPLIER_RETURN">Devolução a fornecedor</option></select></label><label>Série<input value={form.series} onChange={event => update("series", event.target.value)} /></label><label>Número<input value={form.number} onChange={event => update("number", event.target.value)} /></label><label>Cliente/fornecedor ID<input value={form.documentType === "SUPPLIER_RETURN" ? form.supplierId : form.customerId} onChange={event => update(form.documentType === "SUPPLIER_RETURN" ? "supplierId" : "customerId", event.target.value)} /></label><label>Lote/número de série<input value={form.lotNumber || form.serialNumber} onChange={event => update("lotNumber", event.target.value)} /></label><label>Validade<input type="date" value={form.expiresAt} onChange={event => update("expiresAt", event.target.value)} /></label></>  }{tab === "procurement" && <label>N.º pedido de cotação<input value={form.requestNumber} onChange={event => update("requestNumber", event.target.value)} /></label>}{(tab === "quotation" || tab === "order") && <><label>N.º documento<input value={tab === "quotation" ? form.quotationNumber : form.orderNumber} onChange={event => update(tab === "quotation" ? "quotationNumber" : "orderNumber", event.target.value)} /></label><label>Fornecedor ID<input value={form.supplierId} onChange={event => update("supplierId", event.target.value)} /></label><label>Custo unitário (cêntimos)<input type="number" min="0" value={form.unitCostCents} onChange={event => update("unitCostCents", event.target.value)} /></label></>}<button onClick={() => void submit()}>Guardar e auditar</button>{tab === "quotation" && <><h3>Comparação de cotações</h3><table><thead><tr><th>Pedido</th><th>Cotação</th><th>Fornecedor</th><th>Produto</th><th>Custo unitário</th><th>Estado</th></tr></thead><tbody>{requests.flatMap(request => request.quotations.flatMap(quotation => quotation.lines.map(line => <tr key={`${quotation.number}-${line.productId}`}><td>{request.number}</td><td>{quotation.number}</td><td>{quotation.supplierId}</td><td>{products.find(product => product.id === line.productId)?.name ?? line.productId}</td><td>{formatKwanza(line.unitCostCents)}</td><td>{quotation.status}</td></tr>)))}{!requests.some(request => request.quotations.length) && <tr><td colSpan={6}>Sem cotações registadas.</td></tr>}</tbody></table></>}</div>}</section>;
}
function Dashboard() {
