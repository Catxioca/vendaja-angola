...
function StockOperations({ online }: { online: boolean }) {
  type Tab = "overview" | "inventory" | "transfer" | "receipt" | "returns" | "procurement";
  ...
  const [form, setForm] = useState({ productId: "", quantity: "1", reference: "", locationId: "", sourceLocationId: "", destinationLocationId: "", purchaseId: "", documentType: "CUSTOMER_RETURN", series: "A", number: "", customerId: "", supplierId: "", lotNumber: "", serialNumber: "", expiresAt: "", requestNumber: "" });
  ...
  const submit = async () => {
    try {
      ...
      else { endpoint = "/api/v1/procurement/requests"; body = { number: form.requestNumber, lines: [{ productId: form.productId, quantity }] }; }
