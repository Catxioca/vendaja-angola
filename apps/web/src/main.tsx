...
function StockOperations({ online }: { online: boolean }) {
  ...
  const load = async () => {
    if (!online) return;
    try {
      const [productResponse, warehouseResponse, balanceResponse, suggestionResponse, purchaseResponse] = await Promise.all([
        fetch(`${api()}/api/v1/products`, { headers: auth }), fetch(`${api()}/api/v1/stock/warehouses`, { headers: auth }),
        fetch(`${api()}/api/v1/stock/balances`, { headers: auth }), fetch(`${api()}/api/v1/stock/reorder-suggestions`, { headers: auth }), fetch(`${api()}/api/v1/purchases`, { headers: auth }),
      ]);
      if (![productResponse, warehouseResponse, balanceResponse, suggestionResponse, purchaseResponse].every(response => response.ok)) throw new Error("Não foi possível carregar stock e compras.");
      const warehouses = await warehouseResponse.json() as Array<{ locations: Array<{ id: string; name: string }> }>;
      setProducts(await productResponse.json()); setLocations(warehouses.flatMap(warehouse => warehouse.locations)); setBalances(await balanceResponse.json()); setSuggestions(await suggestionResponse.json()); setPurchases(await purchaseResponse.json());
